                  ...
                  >
                    <Icon
                      className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors duration-300 ${
                        isActive && !isCollapsed ? "text-white" : "text-gray-600 group-hover:text-blue-600"
                      }`}
                    />
                  </div>
                  {sidebarExpanded || (isMobile && mobileMenuOpen) ? (
          ...
          <header className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 sm:gap-6 min-w-0 flex-1">
                ...
                <button
                  onClick={() => (!isMobile ? setSidebarExpanded((v) => !v) : undefined)}
                ...
                >
                  <div className="w-5 h-5 flex flex-col justify-center gap-1">
                    <div className={`h-0.5 bg-gray-600 rounded transition-all duration-300 ${!sidebarExpanded ? "rotate-45 translate-y-1" : ""}`}></div>
                    <div className={`h-0.5 bg-gray-600 rounded transition-all duration-300 ${!sidebarExpanded ? "opacity-0" : ""}`}></div>
                    <div className={`h-0.5 bg-gray-600 rounded transition-all duration-300 ${!sidebarExpanded ? "-rotate-45 -translate-y-1" : ""}`}></div>
                  </div>
                </button>
