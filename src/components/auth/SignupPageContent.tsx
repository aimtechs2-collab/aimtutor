...
type CountryRow = { code: string; country: string; name: string; flag: string };
...
export function SignupPageContent({ country, region, city }: GeoAuthParams) {
  ...
  const [countryCodes, setCountryCodes] = useState<CountryRow[]>([]);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const refresh_token = res.data?.refresh_token as string | undefined;
      ...
      setOtpError(
        ax.response?.data?.message || ax.response?.data?.error || ax.message || "Invalid OTP. Please try again.",
      );
      setOtp(["", "", "", "", "", ""]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    api
      .post("/api/v1/auth/register", apiData)
      .then((response) => {
        setIsLoading(false);
      ...
      .catch((error: unknown) => {
        ...
        const errorMessage =
          ax.response?.data?.error || ax.response?.data?.message || ax.message || "Registration failed. Please try again.";
  ...
  const selectedCountry =
    ...
    ({ code: "+91", country: "IN", flag: "🇮🇳", name: "India" } as CountryRow);
    ...
    <>
      ...
      >
        <div className="w-full max-w-lg">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 lg:p-8 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-purple-400/10 to-blue-400/10 rounded-full -translate-y-12 translate-x-12"></div>
            <div className="absolute bottom-0 left-0 w-20 h-20 bg-gradient-to-tr from-blue-400/10 to-pink-400/10 rounded-full translate-y-10 -translate-x-10"></div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onBlur={handleBlur}
                        className={`w-full pl-9 pr-4 py-2.5 text-sm border-2 rounded-xl bg-gray-50/50 transition-all duration-200 focus:outline-none focus:bg-white ${
                  ...
                  <div className="flex gap-2">
                    <div className="relative w-28">
                      <button
                        ...
                        className={`w-full px-2.5 py-2.5 text-sm border-2 rounded-xl bg-gray-50/50 transition-all duration-200 focus:outline-none focus:bg-white flex items-center justify-between ${
                          errors.phone && touched.phone ? "border-red-300" : "border-gray-200 hover:border-purple-300"
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`h-3.5 w-3.5 text-gray-400 transition-transform ${showCountryDropdown ? "rotate-180" : ""}`}
                        />
                      </button>
                          ...
                          <div className="overflow-y-auto max-h-48 custom-scrollbar">
                            {filteredCountries.map((c) => (
                              <button
                                ...
                                className={`w-full px-3 py-2 text-left hover:bg-purple-50 flex items-center gap-2 transition-colors ${
                                  formData.countryCode === c.code ? "bg-purple-100" : ""
                                }`}
                              >
                                <span className="text-base flex-shrink-0">{c.flag}</span>
                                <span className="text-xs font-medium text-gray-700 flex-shrink-0 w-12">{c.code}</span>
                                <span className="text-xs text-gray-500 truncate flex-1">{c.name}</span>
                              </button>
                            ))}
                            {filteredCountries.length === 0 && (
                              <div className="px-3 py-6 text-center text-xs text-gray-500">No countries found</div>
                            )}
                          </div>
                        </div>
                      )}
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      onBlur={handleBlur}
                      rows={2}
                      className={`w-full pl-9 pr-4 py-2.5 text-sm border-2 rounded-xl bg-gray-50/50 transition-all duration-200 focus:outline-none focus:bg-white resize-none ${
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        onBlur={handleBlur}
                        className={`w-full pl-9 pr-10 py-2.5 text-sm border-2 rounded-xl bg-gray-50/50 transition-all duration-200 focus:outline-none focus:bg-white ${
