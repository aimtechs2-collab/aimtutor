// GlobalLocationSwitcher.jsx - SEO Optimized with react-head
import React, { useEffect, useMemo, useState } from "react";
import { Link, useLocation } from "react-router-dom";
import { Title, Meta, Link as HeadLink } from "react-head";
import { worldRegions } from "../utils/worldRegions";

// Continent SEO metadata
const continentMeta = {
  "Europe": { 
  ...
  return (
    <a
      ...
      className="rounded-full border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 
                 shadow-sm transition-all hover:border-blue-500 hover:bg-blue-600 
                 hover:text-white hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-300"
...
const CountrySection = ({ country, userCountry }) => {
  const isUserCountry = userCountry === country.code;
  // const countryUrl = `/${country.code}`;
      ...
      <div className="mb-3 flex items-center gap-2">
        <h3 className="font-semibold text-gray-900" itemProp="name">
          <Link 
            ...
            title={`All software training locations in ${country.name}`}
          >
            {country.name}
          </Link>
        </h3>
        ...
        <div className="flex flex-wrap gap-2">
          {country.cities?.length > 0 ? (
            country.cities.map((city) => (
              <CityLink key={city.code} city={city} country={country} />
            ))
          ) : (
...
const ContinentSection = ({ continent, countries, userCountry }) => {
    ...
    <section className="mb-10" aria-labelledby={`continent-${continent.replace(/\s+/g, '-')}`}>
      <header className="mb-4 border-b border-gray-200 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 
              id={`continent-${continent.replace(/\s+/g, '-')}`}
...
const generateStructuredData = (continents) => {
...
export default function GlobalLocationSwitcher() {
  ...
  const continents = useMemo(() => worldRegions, []);
  ...
  return (
    <>
      {/* SEO Head Tags using react-head */}
      <Title>Software Training Courses in 200+ Cities Worldwide | Professional IT Training</Title>
      
      <Meta 
        name="description" 
        content="Expert-led software training courses in your city. Choose from 200+ locations across Europe, Asia, Americas, and Middle East. Onsite and online IT courses available." 
      />
      <Meta 
        name="keywords" 
        content="software training, IT courses, programming courses, corporate training, onsite training, instructor-led courses, Python training, Java courses, cloud computing" 
      />
      <Meta name="robots" content="index, follow" />
      <Meta name="author" content="Your Training Company" />
      
      {/* Open Graph Tags */}
      <Meta property="og:title" content="Software Training Courses in 200+ Cities Worldwide" />
      <Meta property="og:description" content="Professional IT training in your city. Expert instructors, hands-on learning, corporate solutions." />
      <Meta property="og:type" content="website" />
      <Meta property="og:url" content={canonicalUrl} />
      <Meta property="og:site_name" content="Your Training Company" />
      
      {/* Twitter Card Tags */}
      <Meta name="twitter:card" content="summary_large_image" />
      <Meta name="twitter:title" content="Software Training Courses in 200+ Cities Worldwide" />
      <Meta name="twitter:description" content="Expert-led IT training in your city. 200+ global locations." />
      
      {/* Canonical URL */}
      <HeadLink rel="canonical" href={canonicalUrl} />

      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
        {/* Structured Data Scripts (outside react-head, in body) */}
        <script
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <header className="mb-10 text-center">
            <h1 className="text-3xl font-extrabold text-gray-900 sm:text-4xl">
              🌍 Professional Software Training Courses in 200+ Cities Worldwide
            </h1>
            <p className="mx-auto mt-4 max-w-3xl text-lg text-gray-700">
              Expert-led IT training courses delivered in your city. From Python to Cloud Computing, 
              Machine Learning to DevOps – choose your location for local schedules, pricing, and online training.
            </p>
            <p className="mt-2 text-sm text-gray-600">
              ⭐ Trusted by 50,000+ professionals • 🏆 Corporate training solutions • 📍 Instructor-led courses
            </p>
          </header>
